...
description: "Generate Guizang-style material illustrations, labeled explanatory visuals, material-styled chart illustrations, and data-first editorial images from articles, notes, product concepts, workplace reports, creator posts, tutorials, school materials, humanities topics, science explanations, screenshots, or chart data. Use when the user asks for 配圖, 帶字插圖, 解釋圖, 圖解插畫, 概念拆解圖, 圖表美化, 資料圖美化, 3D 圖表, 彙報配圖, 內容配圖, 小學課文配圖, 生物/化學/物理解釋圖, 人文類配圖, process/loop/system diagrams, or wants GPT-Image / image generation to create supporting images that can sit inside social cards, docs, slides, PPTs, or posts. Default language: Traditional Chinese labels inside generated images."
---
...
- Prefer concrete labels over abstract nouns. Use `使用者提示`, `AI 執行`, `結果檢查`, `下一輪`; not `輸入階段`, `執行階段`, `驗證階段`.
